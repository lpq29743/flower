        ...
        <form class="form-horizontal" action="orderPost" method="post" enctype="multipart/form-data"
            ...
            <div class="form-group">
                ...
                @foreach($customers as $customer)
                    <div class="radio">
                        <label>
                            <input type="radio" name="custID" id="custID" value="{{ $customer->custID }}"
                                   @if($customer->cdefault == 1)
                                   checked
                                    @endif
                            >
                            {{ $customer->sname }}，{{ $customer->mobile }}，{{ $customer->address }}，{{ $customer->zip }}
                            @if($customer->cdefault == 1)
                                默认地址
                            @else
                                <a href="customerUpdate/{{ $customer->custID }}" style="margin-right: 5px;">设为默认地址</a>
                                <a href="customerDelete/{{ $customer->custID }}">删除</a>
                            @endif
                        </label>
                    </div>
                @endforeach
